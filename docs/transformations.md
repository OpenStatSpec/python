...
applies; create/replace order alternated by repetition. Both sizes repeated
this input cycle in order:
`(1,1)`, `(1,NULL)`, `(NULL,1)`, `(0,1)`, `(2,2)`, `(0.1,0)`, `(-9,1)`,
`(NULL,0)`. Each fixture had numeric `source_a` and `source_b`; `source_a`
carried label `Source A` and missing rule `99`, while `source_b` carried label
